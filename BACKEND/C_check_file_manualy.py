...
INPUT_JSON_PATH = SCRIPT_DIR / "CA_check_file_manualy.json"
BACKLIST_JSON_PATH = SCRIPT_DIR / "CB_BLACKLIST.json"
WHITELIST_JSON_PATH = SCRIPT_DIR / "CC_WHITELIST.json"
...
def _positive_float_setting(name: str, default: float) -> float:
    value: Any = getattr(settings, name, default) if settings is not None else default
...
def main() -> int:
    ...
    total_input_urls = 0
    total_whitelist_urls = 0
    total_backlist_urls = 0

    print("Source files to process: 1")
    print(
    ...
    print(f"Backlist:  {BACKLIST_JSON_PATH}")
    print(f"Whitelist: {WHITELIST_JSON_PATH}")
    if not INPUT_JSON_PATH.exists():
        print("SKIP: input file not found.")
        return 1

    try:
        urls = load_urls_from_json(INPUT_JSON_PATH)
    ...
    if total == 0:
        save_urls_to_json(BACKLIST_JSON_PATH, [])
        save_urls_to_json(WHITELIST_JSON_PATH, [])
        print("No URLs found. Saved empty outputs.")
        return 0
    ...
    whitelist_urls = [row.url for row in valid_rows]
    backlist_urls = [row.url for row in failed_rows]
    save_urls_to_json(WHITELIST_JSON_PATH, whitelist_urls)
    save_urls_to_json(BACKLIST_JSON_PATH, backlist_urls)
    total_whitelist_urls = len(whitelist_urls)
    total_backlist_urls = len(backlist_urls)

    print(f"Processed: {processed_total}/{total}")
    print(f"Done: {len(valid_rows)}/{total} URL(s) passed playback-risk check.")
    print(f"Saved whitelist URLs to: {WHITELIST_JSON_PATH}")
    print(f"Saved backlist URLs to: {BACKLIST_JSON_PATH}")
    if valid_rows:
    ...
    print("\n=== Summary ===")
    print("Source files processed: 1")
    print(f"Input URLs total: {total_input_urls}")
    print(f"Working URLs total: {total_whitelist_urls}")
    print(f"Failed URLs total: {total_backlist_urls}")
    print(f"Whitelist saved to: {WHITELIST_JSON_PATH}")
    print(f"Backlist saved to: {BACKLIST_JSON_PATH}")
