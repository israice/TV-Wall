...
SOURCE_JSON_FILE = Path("DATA/CHECK/TEMP_CHECKED.json").resolve()
TARGET_JSON_FILE = Path("DATA/LISTS/ALL.json").resolve()
...
def main() -> int:
    ...
    max_workers = min(get_max_workers(), total or 1)
    ...
    print(f"Checking {total} URL(s) from: {SOURCE_JSON_FILE}")
    print(
        f"Workers: {max_workers}, timeout: {timeout_seconds}s, retries: {retries}, "
    ...
    if output_path.exists():
        ...
        if isinstance(existing_raw, list):
            existing_urls = [item.strip() for item in existing_raw if isinstance(item, str) and item.strip()]
    ...
    print(f"Added to ALL.json: {added_count}")
    print(f"ALL total now: {len(merged_urls)}")
    print(f"Total elapsed: {total_elapsed:.2f}s")
    print(f"Saved merged URLs to: {output_path}")
    if valid_rows:
